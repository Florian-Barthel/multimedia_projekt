...
with tf.Session() as sess:
    ...
    adjustments_loss = graph.adjustments_loss_working(adjustments, gt_tensor, labels_tensor, anchor_grid_tensor)

    anchor_grid_adjusted = dataUtil.calculate_adjusted_anchor_grid_working(anchor_grid_tensor, adjustments)

    if config.use_bounding_box_regression:
        total_loss = probabilities_loss + adjustments_loss / 2
    else:
